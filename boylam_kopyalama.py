...
import zipfile
import subprocess
from tkinter import Tk, Label, Entry, Button, Text, Scrollbar, messagebox, filedialog
...
# Fonksiyonlar
def connect_to_computer(computer, user, username, password, log_text):
...
def copy_file_to_desktop(computer, user, source, username, password, log_text):
    ...
    try:
        connect_to_computer(computer, user, username, password, log_text)
        shutil.copy(source, dest)
        log_text.insert("end", f'{source} başarıyla {dest} konumuna kopyalandı.\n')
    except Exception as e:
        log_text.insert("end", f'Hata: {source} dosyası kopyalanırken bir hata oluştu. {e}\n')
    finally:
        disconnect_from_computer(computer, user, log_text)
...
def extract_zip_to_folder(computer, user, source, dest_folder, username, password, log_text):
    ...
    dest_path = fr'\\{computer}\{dest_folder}'
    try:
        connect_to_computer(computer, user, username, password, log_text)
        with zipfile.ZipFile(source, 'r') as zip_ref:
            zip_ref.extractall(dest_path)
        log_text.insert("end", f'{source} dosyasının içeriği başarıyla {dest_path} konumuna çıkarıldı.\n')
    except Exception as e:
        log_text.insert("end", f'Hata: {source} dosyasının içeriği çıkarılırken bir hata oluştu. {e}\n')
    finally:
        disconnect_from_computer(computer, user, log_text)
...
# Arayüz kurulumu
root = Tk()
root.title("Boylam Sınıfı Dosya Aktarım Aracı")
